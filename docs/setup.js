dojo.provide("dools.docs.setup");

dools.docs.setup = {
	// summary: This class provides all the setup details for the docs analytics part of the uxebu.docs package.
	// description: If you e.g. want to override the mapping of moduleNames to file names,
	...
	module2FilesMap:{
		...
		"dojo.Color":{requires:["dojo.colors"], files:[dojo.moduleUrl("dojo", "colors.js")]},
...
// ??????mmmmmmmmhhh how can we di this? problem we can not do reflection on NodeList-fx  "dojo.NodeList-fx":{requires:["dojo.NodeList-fx"], files:[dojo.moduleUrl("dojo._base", "NodeList.js"), dojo.moduleUrl("dojo", "NodeList-fx.js")]},
		...
		"dojox.analytics":{requires:["dojox.analytics"], files:[dojo.moduleUrl("dojox", "analytics.js"), dojo.moduleUrl("dojox.analytics", "_base.js")]},
		"dojox.lang.aspect":{
			requires:[
				"dojox.lang.aspect",
...
// TODO I dont want this!!!!!!!!! abstract out the setup.js stuff too pffff
"Accelerometer":{
	files:[
		dojo.moduleUrl("phonegap/android/js", "accelerometer.js"),
	]
},
"Compass":{
	files:[
		dojo.moduleUrl("phonegap", "compass.js"),
	]
},
"Geolocation":{
	files:[
		dojo.moduleUrl("phonegap", "geolocation.js"),
	]
}
	},
	
	_loadableModuleNamesCache:{},
	getLoadableModuleName:function(/*String*/moduleName, /*String*/methodName, /*Bool?*/preventCache){
		// summary: Get all the module names that can be used in dojo.require() to load moduleName.
		// description: E.g. a moduleName like "dojo.string.trim" we can not dojo.require
		...
		// returns: An array of module names that can be required. An array because
		// 		sometimes multiple modules have to be loaded, see "dojo.cldr" in the module2FilesMap.
		// examples:
		// 		>>> dools.docs.setup.getLoadableModuleName("dojo.string.trim") == "dojo.string"
		// 		true
		// 		>>> dools.docs.setup.getLoadableModuleName("dojo.string", "trim") == "dojo.string"
		// 		true
		// 		>>> dools.docs.setup.getLoadableModuleName("dojo.query") == "dojo.query"
		// 		true
		...
		if (typeof this.module2FilesMap[m]!="undefined"){
			ret = m;
		} else {
			var parts = m.split("."),
				url, src,
			...
			while(parts.length>1){
				...
				if (src == null){
					...
					if (typeof this.module2FilesMap[possibleModuleName] == "undefined"){
	...
	getSourceFiles:function(/*String*/moduleName, /*String?*/methodName){
		// summary: Get an array of all the source files that are needed for moduleName.
		...
		var m = methodName ? moduleName + "." + methodName : moduleName,
			loadableModuleName = this.getLoadableModuleName(moduleName, methodName);
		if (typeof this.module2FilesMap[loadableModuleName]=="undefined"){
			var parts = loadableModuleName.split("."),
				url = dojo.moduleUrl(parts.slice(0, -1).join("."), parts[parts.length-1]+".js").toString();
			return [url];
		} else {
			this.load(moduleName, methodName);
	...
	getSourceCode:function(/*String*/moduleName, /*String?*/methodName){
		// summary: Return the source code of all the source files configured for moduleName.
		var files = this.getSourceFiles(moduleName, methodName),
			ret = {},
			i, l=files.length;
		for (i=0; i<l; i++){
	...
	load:function(/*String*/moduleName, /*String?*/methodName){
	...
	_loadSourceCode:function(url, preventCache){
		// summary: Load the source file synchronously.
