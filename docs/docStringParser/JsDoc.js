...
console.log('parsed = ', parsed);
			if (typeof ret.param != "undefined"){
				ret.parameters = this._parseParameters(parsed.param);
				delete ret.param;
			}
			return ret;
		},
		...
		_parseParameters:function(parsedParams){
			...
			for (var j=0, l=params.length, p; j<l; j++){
				...
				var res = p.match(paramRegExp),
					param = {name:"", summary:"", datatype:undefined};
				if (res==null){
					dojo.mixin(param, {description:p});
				} else {
					dojo.mixin(param, {name:res[2], description:p.replace(paramRegExp, ""), datatype:res[1]});
				}
				ret.push(param);
			}
			return ret;
